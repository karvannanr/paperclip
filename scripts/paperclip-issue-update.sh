...
Examples:
  ...
  scripts/paperclip-issue-update.sh --issue-id "$STAPLER_TASK_ID" --status done --dry-run <<'MD'
  Done
...
if [[ -z "$issue_id" ]]; then
  printf 'Missing issue id. Pass --issue-id or set STAPLER_TASK_ID.\n' >&2
  exit 1
fi
...
if [[ -z "${STAPLER_API_URL:-}" || -z "${STAPLER_API_KEY:-}" || -z "${STAPLER_RUN_ID:-}" ]]; then
  printf 'Missing STAPLER_API_URL, STAPLER_API_KEY, or STAPLER_RUN_ID.\n' >&2
  exit 1
fi

curl -sS -X PATCH \
  "$STAPLER_API_URL/api/issues/$issue_id" \
