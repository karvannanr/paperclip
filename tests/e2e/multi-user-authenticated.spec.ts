...
const BASE = process.env.STAPLER_E2E_BASE_URL ?? "http://127.0.0.1:3105";
const DATA_DIR = process.env.STAPLER_E2E_DATA_DIR ?? process.env.STAPLER_HOME;
const CONFIG_PATH = process.env.STAPLER_E2E_CONFIG_PATH ?? path.resolve(process.cwd(), ".paperclip/config.json");
...
function createBootstrapInvite() {
  if (!DATA_DIR) {
    throw new Error("STAPLER_E2E_DATA_DIR or STAPLER_HOME is required for authenticated bootstrap tests");
  }
