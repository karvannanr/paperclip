...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@stapler/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@stapler/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@stapler/adapter-utils").TranscriptEntry[];
