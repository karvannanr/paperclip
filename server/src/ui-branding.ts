const FAVICON_BLOCK_START = "<!-- STAPLER_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- STAPLER_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- STAPLER_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- STAPLER_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.STAPLER_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.STAPLER_WORKTREE_NAME) ?? nonEmpty(env.STAPLER_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.STAPLER_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
