...
vi.mock("jsdom", () => {
  throw new Error("require() of ES Module not supported");
});
...
describe("SVG upload fallback sanitization", () => {
  afterEach(() => {
  ...
  it("sanitizes SVG uploads without loading jsdom", async () => {
    ...
    const res = await request(app)
      ...
      .attach(
        "file",
        Buffer.from(
          "<svg xmlns='http://www.w3.org/2000/svg' onload='alert(1)' style='background:url(https://evil.example/x)'><script>alert(1)</script><a href='https://evil.example/'><circle cx='12' cy='12' r='10'/></a></svg>",
        ),
        "logo.svg",
      );
