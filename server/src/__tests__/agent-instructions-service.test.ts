...
describe("agent instructions service", () => {
  const originalPaperclipHome = process.env.STAPLER_HOME;
  const originalPaperclipInstanceId = process.env.STAPLER_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalPaperclipHome === undefined) delete process.env.STAPLER_HOME;
    else process.env.STAPLER_HOME = originalPaperclipHome;
    if (originalPaperclipInstanceId === undefined) delete process.env.STAPLER_INSTANCE_ID;
    else process.env.STAPLER_INSTANCE_ID = originalPaperclipInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    ...
    process.env.STAPLER_INSTANCE_ID = "test-instance";
