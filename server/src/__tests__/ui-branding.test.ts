...
<head>
    <!-- STAPLER_RUNTIME_BRANDING_START -->
    <!-- STAPLER_RUNTIME_BRANDING_END -->
    <!-- STAPLER_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from STAPLER_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ STAPLER_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ STAPLER_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ STAPLER_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      STAPLER_IN_WORKTREE: "true",
      STAPLER_WORKTREE_NAME: "paperclip-pr-432",
      STAPLER_WORKTREE_COLOR: "#4f86f7",
    });
