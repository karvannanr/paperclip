...
- `paperclipPublicUrl` (optional override; else `STAPLER_PUBLIC_URL` env)
...
Cursor agents run remotely, so we cannot inject local env like `STAPLER_API_KEY`.
...
2. `process.env.STAPLER_PUBLIC_URL`
