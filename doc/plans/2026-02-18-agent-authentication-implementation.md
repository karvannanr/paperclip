...
- New config/env settings:
  - `STAPLER_AGENT_JWT_SECRET`
  - `STAPLER_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `STAPLER_AGENT_JWT_ISSUER` (default: `paperclip`)
  - `STAPLER_AGENT_JWT_AUDIENCE` (default: `paperclip-api`)
...
1. In:
   ...
   inject `STAPLER_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `STAPLER_API_KEY`, do not overwrite it.
- Continue injecting:
  - `STAPLER_AGENT_ID`
  - `STAPLER_COMPANY_ID`
  - `STAPLER_API_URL`
...
- Local adapters authenticate without manual `STAPLER_API_KEY` config.
