...
type PaperclipSprite = (typeof STAPLER_SPRITES)[number];
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = STAPLER_SPRITES[(baseCol + baseRow) % STAPLER_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = STAPLER_SPRITES[Math.floor(Math.random() * STAPLER_SPRITES.length)]!;
