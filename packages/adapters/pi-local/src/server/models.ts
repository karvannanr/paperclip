...
import { asString, runChildProcess } from "@stapler/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.STAPLER_PI_COMMAND === "string" &&
    process.env.STAPLER_PI_COMMAND.trim().length > 0
      ? process.env.STAPLER_PI_COMMAND.trim()
      : "pi";
