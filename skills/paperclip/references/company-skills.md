...
```sh
curl -sS -X POST "$STAPLER_API_URL/api/companies/$STAPLER_COMPANY_ID/skills/import" \
...
```sh
curl -sS -X POST "$STAPLER_API_URL/api/companies/$STAPLER_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $STAPLER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$STAPLER_API_URL/api/companies/$STAPLER_COMPANY_ID/skills" \
  -H "Authorization: Bearer $STAPLER_API_KEY"
```
...
```sh
curl -sS "$STAPLER_API_URL/api/companies/$STAPLER_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $STAPLER_API_KEY"

curl -sS "$STAPLER_API_URL/api/companies/$STAPLER_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $STAPLER_API_KEY"
```
...
```sh
curl -sS -X POST "$STAPLER_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $STAPLER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$STAPLER_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $STAPLER_API_KEY"
```
...
```sh
curl -sS -X POST "$STAPLER_API_URL/api/companies/$STAPLER_COMPANY_ID/agents" \
  -H "Authorization: Bearer $STAPLER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
