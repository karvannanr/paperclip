...
```bash
npx @googlarz/stapler issue create \
...
```bash
npx @googlarz/stapler heartbeat run --agent-id "$STAPLER_AGENT_ID"
```

3. Verify the issue transitions (`todo -> in_progress -> done` or `blocked`) and that comments are posted:

```bash
npx @googlarz/stapler issue get <issue-id-or-identifier>
```

4. Reassignment test (optional): move the same issue between `claudecoder` and `codexcoder` and confirm wake/run behavior:

```bash
npx @googlarz/stapler issue update <issue-id> --assignee-agent-id <other-agent-id> --status todo
```
